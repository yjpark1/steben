...
from baselines.dimes_tsp.torch_sampling_py.tsp import *
...
def tsp_greedy(graph, par, sample_size, best = True):
    ys, tours = tsp_greedy_py(graph.x, graph.degs, graph.edge_index[1], par, sample_size)
    if best:
        y, i = ys.min(dim = 0)
...
def tsp_softmax(graph, par, sample_size, y_bl = None, best = True):
    ys, tours = tsp_softmax_py(graph.x, graph.degs, graph.edge_index[1], par, sample_size, np.nan if y_bl is None else y_bl)
    if best:
        y, i = ys.min(dim = 0)
        return y.detach().clone(), tours[i].detach().clone()
    else:
...
def tsp_softmax_grad(graph, par, sample_size, y_bl = None):
    ys, grad = tsp_softmax_grad_py(graph.x, graph.degs, graph.edge_index[1], par, sample_size, np.nan if y_bl is None else y_bl)
