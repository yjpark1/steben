...
class GNNLayer(nn.Module):
  ...
  def aggregate(self, Vh, graph, gates, mode=None, edge_index=None, sparse=False):
    ...
    graph_eye = torch.eye(graph.shape[1], dtype=torch.int64, device=graph.device).unsqueeze(0)
    graph = graph + graph_eye

    gates_neighbor = gates * graph.unsqueeze(-1)

    Vh = gates_neighbor * Vh  # B x V x V x H
...
class EdgeCostEmbedding(nn.Module):
  def __init__(self, out_features=64, normalize=True):
    super().__init__()
  ...
  def forward(self, x):
    ...
    x_mask[torch.where(x==0)] = 1
    x = x + x_mask
    
    self.linear = self.linear.to(x.device)
    # check_x = (x>0)
    # self.linear_empty = 64dim
    # x = self.linear(x)*check_x + self.linear_empty * (1-check_x)
    x = self.linear(x)
    return x.view(batch_size, num_nodes, num_nodes, self.out_features)
      
class EdgeExistEmbedding(nn.Module):
  def __init__(self, out_features=64, normalize=True):
    super().__init__()
    self.out_features = out_features
    self.linear = nn.Linear(1, out_features)
  ...
  def forward(self, x):
    batch_size, num_nodes, _ = x.size()
    x_mask = (x != 0).float()
    x = x_mask.view(batch_size, num_nodes * num_nodes, 1)
    self.linear = self.linear.to(x.device)
    x = self.linear(x)
...
class GNNEncoder(nn.Module):
  ...
  def __init__(self, n_layers, hidden_dim, out_channels=1, aggregation="sum", norm="layer",
    ...
    self.edge_embed = nn.Linear(hidden_dim * 3, hidden_dim)
    ...
    if not node_feature_only:
      ...
      self.edge_cost_embed = EdgeCostEmbedding(hidden_dim, normalize=True)
      self.edge_exist_embed = EdgeExistEmbedding(hidden_dim, normalize=False)
    else:
  ...
  def dense_forward(self, x, graph, timesteps, edge_index=None, edge_cost=None):
    """
    Args:
    ...
    cost_e = self.edge_cost_embed(edge_cost)
    exist_e = self.edge_exist_embed(edge_cost)
    adj_e = self.edge_pos_embed(graph)
    e = self.edge_embed(torch.cat((adj_e, cost_e, exist_e), dim=-1))
    # e = self.edge_embed(adj_e)
    time_emb = self.time_embed(timestep_embedding(timesteps, self.hidden_dim))
    
    # graph = torch.ones_like(graph).long()
    graph_mask = (edge_cost != 0).long()
    ...
    for layer, time_layer, out_layer in zip(self.layers, self.time_embed_layers, self.per_layer_out):
      ...
      # x, e = layer(x, e, graph_mask, mode="direct")
  ...
  def forward(self, x, timesteps, graph=None, edge_index=None, edge_cost=None):
    if self.node_feature_only:
      ...
      if self.sparse:
        ...
        return self.dense_forward(x, graph, timesteps, edge_index, edge_cost)
