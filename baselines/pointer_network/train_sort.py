...
parser.add_argument('--max-length', type=int, default=20, help='maximum length of sequences (default: 20)')
parser.add_argument('--train-samples', type=int, default=256, help='number of samples in train set (default: 100000)')
...
parser.add_argument('--no-cuda', default=True, help='disables CUDA training')
parser.add_argument('--seed', type=int, default=1, help='random seed (default: 1)')
parser.add_argument('--bi', type=bool, default=False, help='random seed (default: 1)')
...
def main():
	...
	model = PointerNet(input_dim=args.high, embedding_dim=args.emb_dim, hidden_size=args.emb_dim, bidirectional=args.bi).to(device)
	...
	for epoch in range(args.epochs):
		...
		for batch_idx, (seq, length, target) in enumerate(train_loader):
			...
			optimizer.zero_grad()
			log_pointer_score, argmax_pointer, mask = model(seq, length+1, max_seq_len)  # concate init embb
